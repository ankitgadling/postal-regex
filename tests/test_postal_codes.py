...
def test_no_duplicate_country_code_or_name():
    ...
    assert (
        not duplicate_codes
    ), f"Duplicate country codes found: {', '.join(duplicate_codes)}"
    assert (
        not duplicate_names
    ), f"Duplicate country names found: {', '.join(duplicate_names)}"
...
def test_postal_code_regex(entry):
    ...
    assert pattern.fullmatch(sample_valid), (
        f"Valid postal code '{sample_valid}' " f"not match {entry['country_name']}"
    )

    assert not pattern.fullmatch(sample_invalid), (
        f"Invalid postal code '{sample_invalid}' "
        f"incorrectly matched {entry['country_name']}"
    )
